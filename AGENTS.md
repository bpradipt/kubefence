...
```
cmd/nono-nri/main.go              entrypoint: flags, kernel check, config, stub.Run
...
internal/nri/profile.go           ResolveProfile — annotation → profile name
...
internal/nri/config.go            Config struct, LoadConfig (TOML), DefaultConfig
internal/nri/kernel.go            CheckKernel — Linux 5.13+ required for Landlock
internal/log/log.go               slog JSON (prod) / text (dev) logger factory
...
**Log capture** — use `newBufLogger(buf)` (defined in `helpers_test.go`) to
capture slog JSON output. Parse with `json.Unmarshal` into `logEntry`:
...
`deploy/kind/e2e.sh` runs 17 checks in 5 tests:
...
```
Pod: nono-nri (kube-system, privileged)
...
(client), not the other way around. The plugin state dir (`/var/run/nono-nri`)
is inside the container's ephemeral filesystem — it is not a hostPath mount.
...
- Kata with nested KVM in kind requires:
  ...
  - Host kernel instead of Kata bundled kernel (Kata 6.18.15 lacks
    `CONFIG_SECURITY_LANDLOCK`; host kernel 6.8+ has it built-in)
  - Custom initrd with host kernel vsock + virtiofs modules insmod'd before
    kata-agent (see `deploy/kind/deploy.sh` for the build steps)
